...
class EGNN_dynamics_QM9(nn.Module):
    def __init__(self, in_node_nf, context_node_nf,
        ...
        if mode == 'egnn_dynamics':
            self.egnn = EGNN(
                ...
                tanh=tanh, norm_constant=norm_constant,
                inv_sublayers=inv_sublayers, sin_embedding=sin_embedding,
                normalization_factor=normalization_factor,
                aggregation_method=aggregation_method)
            self.in_node_nf = in_node_nf
        elif mode == 'qegnn_dynamics':
            self.qegnn = QEGNN(
                in_node_nf=in_node_nf + context_node_nf, in_edge_nf=1,
                hidden_nf=hidden_nf, device=device, act_fn=act_fn,
                n_layers=n_layers,
    ...
    def _forward(self, t, xh, node_mask, edge_mask, context=None):
        """
        use _forward as forward's alias, ensure compatibility with en_diffusion.py's call
        """
        return self.forward(t, xh, node_mask, edge_mask, context)
    ...
    def forward(self, t, xh, node_mask, edge_mask, context=None):
        ...
        elif self.mode == 'qegnn_dynamics':
            h_final, x_final = self.qegnn(h, x, edges, node_mask=node_mask, edge_mask=edge_mask)
            vel = (x_final - x) * node_mask
