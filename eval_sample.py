...
def main():
    ...
    # 先存molecules/中的前n_samples个.txt文件, 命名为molecule_xxx.txt
    sample_different_sizes_and_save(
        args, eval_args, device, flow, nodes_dist,
        dataset_info=dataset_info, n_samples=30) 

    print('Sampling stable molecules.')
    # 再sample n_tries个分子,验证是否稳定,直到存n_samples个稳定分子在molecules/中, 命名为molecule_stable_xxx.txt
    ...
    # 采样n_chains个chain, 每个chain实际上sample至多n_tries次直到找到一个稳定分子, 否则展示最后一次try的sample结果, 并保存到eval/chain_x/中, 命名为chain_xxx.txt (注意一个chain是一个分子的形成过程)
    save_and_sample_chain(
        args, eval_args, device, flow,
        n_tries=eval_args.n_tries, n_nodes=eval_args.n_nodes,
        dataset_info=dataset_info, num_chains=2) 
