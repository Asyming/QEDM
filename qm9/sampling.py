...
def sample_chain(args, device, flow, n_tries, dataset_info, prop_dist=None):
    n_samples = 1
    if (args.dataset == 'qm9' or args.dataset == 'qm9_second_half' or args.dataset == 'qm9_first_half') & (args.remove_h == False) & (args.filter_n_atoms == None):
        n_nodes = 19
    elif (args.dataset == 'qm9' or args.dataset == 'qm9_second_half' or args.dataset == 'qm9_first_half') & (args.remove_h == True) & (args.filter_n_atoms == None):
        n_nodes = 9
    elif args.filter_n_atoms != None:
...
def sample(args, device, generative_model, dataset_info,
           prop_dist=None, nodesxsample=torch.tensor([]), context=None,
           fix_noise=False):
    # max_n_nodes = dataset_info['max_n_nodes']  # this is the maximum node_size in QM9 ##debug
    if (args.filter_n_atoms == None) & (args.remove_h == False):
        max_n_nodes = 29
    elif (args.filter_n_atoms == None) & (args.remove_h == True):
        max_n_nodes = 9
    elif args.filter_n_atoms != None:
        max_n_nodes = args.filter_n_atoms
    assert int(torch.max(nodesxsample)) <= max_n_nodes ##debug
...
def sample_sweep_conditional(args, device, generative_model, dataset_info, prop_dist, n_nodes=19, n_frames=100): # 暂未兼容qgnn
