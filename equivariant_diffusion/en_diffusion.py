...
def gaussian_KL_for_dimension(q_mu, q_sigma, p_mu, p_sigma, d):
    """Computes the KL distance between two normal distributions.
        ...
        """
    # 修改: 添加对q_sigma维度的处理
    if len(q_sigma.size()) > 1:
        # 如果q_sigma是多维的，取其平均值或者第一维
        q_sigma = q_sigma.squeeze(1).mean(dim=-1)

    if len(q_sigma.size()) == 0:
        q_sigma = q_sigma.unsqueeze(0)
    
    # 处理p_sigma的维度 - 关键是这部分也需要同样处理
    if len(p_sigma.size()) > 1:
        p_sigma = p_sigma.squeeze(1).mean(dim=-1)
...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def kl_prior(self, xh, node_mask):
        ...
        # # 添加打印语句，查看形状
        # print(f"[DEBUG] mu_T_x shape: {mu_T_x.shape}, sigma_T_x shape: {sigma_T_x.shape}")
        
        # 确保sigma_T_x的维度正确
        if len(sigma_T_x.size()) > 1:
            sigma_T_x = sigma_T_x.mean(dim=-1)
    ...
    def sample_chain(self, n_samples, n_nodes, node_mask, edge_mask, context, keep_frames=None):
        ...
        if keep_frames is None:
            keep_frames = self.T
        # else:
        #     assert keep_frames <= self.T ##debug
